...
<div class="flex flex-col">
    @if(Route::has('login'))
        <div
            ...
            @else
                <a href="{{ route('login') }}"
                   class="no-underline hover:underline text-red-100 text-sm font-normal text-teal-800 uppercase">{{ __('Login') }}</a>
                @if (Route::has('register'))
                    <a href="{{ route('register') }}"
                       class="no-underline text-yellow-700 hover:underline text-sm font-normal text-teal-800 uppercase">{{ __('Register') }}</a>
                @endif
            @endauth
        </div>
    @endif
    ...
    <div class="min-h-screen flex items-center justify-center">
        <div class="flex flex-col justify-around h-full">
            <div>
                ...
                <ul class="flex flex-col text-blue space-y-2 sm:flex-row sm:flex-wrap sm:space-x-8 sm:space-y-0">
                    <li>
